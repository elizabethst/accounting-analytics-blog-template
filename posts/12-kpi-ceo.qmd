...
executive_kpis <- tribble(
  ...
  "Revenue ($M)",    775,      760,     721,         "Ahead",
  "EBITDA Margin %", 17.5,     18.5,    16.7,        "Behind",
  "Market Share %",  20.7,     21.7,    19.4,        "Behind",
  "Customer NPS",    68,       72,      63,          "Behind",
  "Stock Price ($)", 126.71,   140,     104.58,      "Behind"
)
...
_____ |>
  ...
  tab_header(
    ...
    subtitle = paste("FY", format(as_date("2026-11-30"), "%Y"), "Performance")
  ) |>
...
performance_data <- executive_kpis |>
  mutate(
    ...
    performance_score = current / target * 100
  )
...
# Build quarterly profit margin data showing improvement trends
profit_metrics <- tribble(
  ~metric,           ~q1_2026, ~q2_2026, ~q3_2026, ~q4_2026,
  "Gross Margin %",  39.3,     39.8,     40.3,     40.8,
  "EBITDA Margin %", 16.8,     17.1,     17.5,     17.8,
  "Net Margin %",    10.5,     10.7,     11,       11.2
)
profit_metrics <- profit_metrics |>
  # Transform data from wide to long format for plotting
  pivot_longer(cols = -metric, names_to = "quarter", values_to = "value") |>
  # Clean up quarter names for better display
  mutate(quarter = str_replace(quarter, "_", " "))
...
cash_flow_data <- tribble(
  ...
  "Operating", 33,  36,  40,  43,
  "Investing", -15, -19, -15, -19,
  "Financing", -17, -17, -20, -17
)
...
# Market share data showing current vs. prior year positions for all major competitors
market_data <- tribble(
  ~company,       ~current_share, ~prior_share,
  "Our Company",  20.7,           19.4,
  "Competitor A", 25.3,           25.7,
  "Competitor B", 17.7,           18,
  "Competitor C", 12.5,           12.1,
  "Others",       23.8,           25.8
)
...
# Competitive benchmarking data showing key metrics across competitors
competitive_data <- tribble(
  ~metric,            ~your_company, ~competitor_a, ~competitor_b, ~competitor_c,
  "Revenue Growth %", 7.6,           6.1,           5.4,           6.8,
  "EBITDA Margin %",  17.5,          18.8,          16.1,          16.8,
  "Market Share %",   20.7,          25.3,          17.7,          12.5,
  "Customer NPS",     68,            74,            61,            66,
  "R&D Spend %",      8.5,           9.2,           7.5,           8
)
...
customer_metrics <- tribble(
  ...
  "NPS",                  68,       72,      "↑",
  "Retention Rate %",     87.3,     90,      "↑",
  "Lifetime Value ($K)",  146,      150,     "↑",
  "Acquisition Cost ($)", 440,      500,     "↓",
  "Upsell Rate %",        21,       25,      "→"
)
...
# Revenue, growth rates, margins, and customer counts for each business segment
segment_data <- tribble(
  ~segment,         ~revenue_m, ~growth_pct, ~margin_pct, ~customers,
  "Enterprise",     375,        10,          23,          143,
  "Mid-Market",     265,        8,           17,          412,
  "Small Business", 135,        4,           16,          2745
)
...
# Operational metrics data comparing actual performance to industry benchmarks
ops_metrics <- tribble(
  ~category,      ~metric,                     ~value, ~benchmark,
  "Productivity", "Revenue per Employee ($K)", 395,    400,
  "Productivity", "Units per Hour",            84,     75,
  "Efficiency",   "Asset Turnover",            1.8,    1.5,
  "Efficiency",   "Inventory Turns",           12.3,   10,
  "Quality",      "Defect Rate %",             1.1,    2,
  "Quality",      "On-Time Delivery %",        93.5,   92
)
...
# Monthly quality metrics data for the first half of the year
quality_trend <- tribble(
  ~month,       ~complaints, ~defects, ~satisfaction,
  "2026-01-01", 44,          1.5,      87,
  "2026-02-01", 41,          1.4,      88,
  "2026-03-01", 39,          1.3,      89,
  "2026-04-01", 36,          1.3,      90,
  "2026-05-01", 34,          1.2,      91,
  "2026-06-01", 31,          1.1,      92
)
...
# R&D innovation pipeline data
# Projects at each stage of development from initial research to market launch
...
digital_projects <- tribble(
  ...
  "Cloud Migration",    80,              "High",     "On Track",
  "AI/ML Platform",     43,              "High",     "Behind",
  "Customer Portal",    95,              "Medium",   "Nearly Complete",
  "Process Automation", 67,              "High",     "On Track",
  "Data Analytics",     30,              "Critical", "At Risk"
)
...
# Projects key financial and market metrics across five years
...
# Scenario analysis data with three potential outcomes
...
scenarios <- tribble(
  ...
  "Best Case",  1007,          21,           20,
  "Base Case",  884,           19.3,         60,
  "Worst Case", 764,           17,           20
)
scenarios <- scenarios |>
  mutate(
