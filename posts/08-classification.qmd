...
set.seed(2026108)          # Reproducible results
```
...
```{r}
#| label: importance
# Extract variable importance (rpart records it on the fitted engine)
tree_importance <- fraud_tree_fit |>
  ...
  mutate(
    Variable = case_when(
      ...
      Variable == "is_after_hours" ~ "After-Hours Transaction",
      Variable == "documentation_score" ~ "Documentation Score",
      Variable == "is_rushed" ~ "Rushed Approval",
      Variable == "vendor_age_days" ~ "Vendor Age (days)",
      TRUE ~ Variable
    )
  )
...
business_impact |>
  ...
  fmt_percent(columns = Value, rows = Metric %in% c("detection_rate", "false_positive_rate", "investigation_rate") , decimals = 0) |>
  fmt_number(columns = Value, rows = Metric %in% c("total_fraud",
  "fraud_detected", "false_positives", "high_risk_count"), decimals = 0)
```
...
4. **Control Improvements**:
   ...
   - Implement automated flags for low _____ values
