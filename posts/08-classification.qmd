...
The decision tree classification model achieves _______% overall accuracy with _______% sensitivity (fraud detection rate) and _______% specificity, enabling auditors to identify _______ of _______ fraudulent transactions in the test data while generating only _______ false positives (Prediction = Fraudulent; Truth = Legitimate), making it a reliable tool for risk-based audit sampling. Variable importance analysis reveals that transaction amount (importance score: _______) and documentation score (importance score: _______) are the strongest fraud predictors, supported by distinctive fraud patterns including _______% new vendor involvement versus _______% for legitimate transactions, _______% weekend processing versus _______%, and an average documentation score of _______ compared to 8.2 for legitimate transactions. Implementation of this model would require investigating only _______% of total transactions (12 high-risk cases) while capturing _______% of actual fraud, significantly improving audit efficiency compared to random sampling, with recommended control enhancements focusing on automated flags for transactions with poor documentation scores, new vendors under _______ days old, and after-hours or weekend processing where fraudulent activity rates are higher than legitimate transactions.
...
no_legit  <- amount_summary |>
  filter(fraud == "Legitimate") |>
  pull(count)

no_fraud  <- amount_summary |>
  filter(fraud == "Fraudulent") |>
  pull(count)
...
2. **Key Fraud Indicators**: The top predictors of fraud were:
