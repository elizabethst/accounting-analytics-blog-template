...
control_tests <- tribble(
  ...
  "SOX-001",   "2026-11-23", 15,           0,         "Pass",
  "SOX-001",   "2026-11-28", 15,           2,         "Fail",
  "SOX-002",   "2026-11-03", 5,            0,         "Pass",
  "SOX-003",   "2026-11-25", 10,           0,         "Pass",
  "SOX-004",   "2026-11-08", 20,           0,         "Pass",
  "SOX-005",   "2026-11-24", 10,           0,         "Pass"
)
...
financial_data <- tribble(
  ...
  "Revenue",             "P&L",     1280000,        1256000,      1139000,     1175000,
  "COGS",                "P&L",     -726000,        -721000,      -680000,     -770000,
  "Operating Expenses",  "P&L",     -352000,        -334000,      -335000,     -345000,
  "Cash",                "BS",      374000,         345000,       332000,      395000,
  "Accounts Receivable", "BS",      400000,         368000,       389000,      360000,
  "Inventory",           "BS",      268000,         272000,       259000,      275000,
  "Accounts Payable",    "BS",      -186000,        -176000,      -165000,     -205000
)
...
journal_entries <- tribble(
  ...
  "JE001",   "2026-11-28", "14:06:00",  "jsmith",  85000,   "Monthly depreciation",       "Asset",
  "JE002",   "2026-11-27", "21:09:00",  "kwilson", 75000,   "Adjust revenue recognition", "Revenue",
  "JE003",   "2026-11-28", "16:10:00",  "temp001", 120000,  "Record payroll accrual",     "Expense",
  "JE004",   "2026-11-30", "23:38:00",  "mjones",  45000,   "Correct inventory error",    "Asset",
  "JE005",   "2026-11-29", "09:36:00",  "temp001", 55000,   "Office supplies",            "Expense",
  "JE006",   "2026-11-28", "03:03:00",  "kwilson", 30000,   "Reverse accrual",            "Reserve",
  "JE007",   "2026-11-28", "10:23:00",  "admin",   15000,   "Book depreciation",          "Asset",
  "JE008",   "2026-11-29", "22:54:00",  "jsmith",  35000,   "Adjust reserve balance",     "Reserve"
)
...
historical_expenses <- tribble(
  ...
  "2026-06", 45700,   12700,     20000,      27700,
  "2026-07", 48100,   12300,     19200,      29200,
  "2026-08", 49000,   12600,     20700,      26700,
  "2026-09", 45300,   13100,     20200,      27300,
  "2026-10", 49000,   12500,     21400,      29200,
  "2026-11", 47200,   12400,     19700,      51200         # Current month with anomaly
)
...
# Days to next filing
next_filing <- as_date("2026-11-30") + days(77)
today = as_date("2026-11-30") + days(15)
...
high_risk_je <- journal_entries |>
  _____(
    days_until_period_end = as.numeric(as_date("2026-11-30") - entry_date)
  ) |>
  _____(
...
normal_ranges <- historical_expenses |>
  ...
  filter(month < floor_date(as_date("2026-11-30"), "month")) |>
...
current_expenses <- historical_expenses |>
  ...
  _____(month == floor_date(as_date("2026-11-30"), "month")) |>
