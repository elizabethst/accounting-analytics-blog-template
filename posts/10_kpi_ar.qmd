...
# Load the AR data
ar_data  <- read_rds("https://estanny.org/data/2026-fall/10-assignment-ar_data.rds")
```

# Executive Summary {.tabset}

As of November 30, 2026, the company faces significant collection risk with _____ (_____) of the _____ total accounts receivable past due, including _____ severely delinquent beyond 90 days concentrated primarily in the _____ and _____ sectors. _____ of ten major customers are classified as high-risk, with credit utilization rates ranging from _____ to _____ above their limits—most notably the customers, _____ and _____—signaling an urgent need to reassess credit policies and enforce collection protocols on these accounts. Collection trends reveal an average days-to-pay of _____ days against standard 30-day terms, with DSO of _____ days indicating deteriorating collection efficiency that requires immediate intervention to prevent further aging. Based on historical collection patterns and current aging the cash collection forecast projects approximately _____ in expected receipts and _____ is at write-off risk.
...
avg_daily_sales <- ar_data |>
  filter(status == "Paid" & payment_date >= current_date - days(90)) |>
  _____(daily_sales = sum(amount) / 90) |>
  pull(daily_sales)
...
past_due_percentage <- _____ / total_ar
...
avg_days_to_pay <- ar_data |>
  _____(status == "Paid" & !is.na(payment_date)) |>
  _____(days_to_pay = as.numeric(payment_date - invoice_date)) |>
  _____(avg_days = mean(days_to_pay, na.rm = TRUE)) |>
  pull(avg_days)
```
...
_____(aging_plot, tooltip = c("x", "y")) |>
...
utilization_plot <- utilization_data |>
  _____(aes(y = utilization_category)) +
...
monthly_collections <- ar_data |>
  ...
  filter(payment_month >= current_date - months(6))
...
timing_plot <- payment_patterns |>
  ...
  ggplot(_____(x = payment_timing, y = n, fill = payment_timing)) +
  geom_col() +
  scale_fill_manual(
    values = c(
...
weekly_forecast <- forecast_data |>
  _____(expected_collection_week <= current_date + weeks(8)) |>
...
weekly_forecast |>
  _____(aes(x = expected_collection_week)) +
  geom_ribbon(
