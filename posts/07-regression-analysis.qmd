...
The revenue forecasting model explains _______% of revenue variance (R²) with a Mean Absolute Percentage Error of just _______%, identifying marketing spend as the primary driver where each $1,000 investment generates $_______ in additional revenue, providing management with a reliable tool for budgeting and resource allocation decisions. The expense analysis reveals a cost structure with $50,365 in fixed costs and $_______ variable cost per production unit, enabling the creation of flexible budgets ranging from $_______ to $_______ depending on activity levels, which supports more accurate variance analysis and cost control. Applying these regression models as analytical procedures for audit purposes identified _______ periods with sales deviations exceeding the 5% materiality threshold, including Month 30 with a _______% negative variance, flagging $_______ in total transactions requiring substantive investigation to determine whether the differences represent estimation error, changed business conditions, or potential misstatement.
...
# Total sales requiring investigation
exceptions |> 
  summarize(total = sum(reported_sales)) |> 
  gt() |> 
  tab_header(title = "Total Sales Requiring Investigation") |>
